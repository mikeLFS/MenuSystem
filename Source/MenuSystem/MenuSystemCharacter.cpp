	...
	if (OnlineSubsystem)
	{
		...
		if (GEngine)
		{
			GEngine->AddOnScreenDebugMessage(
				-1, 15.f,
				FColor::Blue,
				FString::Printf(TEXT("Subsystem Found : %s"), *OnlineSubsystem->GetSubsystemName().ToString())
			);
		}
	}
}
...
void AMenuSystemCharacter::OnFindSessionComplete(bool bWasSuccessfull)
{
	if (SessionSearch->SearchResults.IsEmpty())
	{
		GEngine->AddOnScreenDebugMessage(
			...
			15.f,
			FColor::Purple,
			FString(TEXT("SearchResults Empty"))
		);
	}
	...
	for (auto &Result : SessionSearch->SearchResults)
	{
		...
		//UE_LOG(LogTemp, Warning , TEXT("Text: %s"), NAME_GameSession)
		}
	}
}
